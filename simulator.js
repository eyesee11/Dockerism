...
function setupVisualBuilder() {
    ...
    const steps = [
        {
            ...
            action: () => {
                ...
                ['file-pkg', 'file-src', 'file-docker'].forEach((id, i) => {
                    ...
                    el.style.transform = `translate(180%, ${50 - (i * 50)}px) rotate(${5 - (i * 5)}deg)`;
                });
    ...
    nextBtn.addEventListener('click', () => {
        ...
        nextBtn.textContent = currentBuildStep < steps.length ? "Next Step" : "Finish";
    });
}
...
function setupNavigation() {
    ...
    const guideSubSections = ['intro', 'concepts', 'vm-vs-container', 'dockerfile'];
...
function setupSimulator() {
    ...
    const scenarios = {
        ...
        masterclass: {
            ...
            steps: [
                { id: 1, title: "Step 1: Create Project", desc: "Create a folder named 'myapp'.", hint: "mkdir myapp" },
                { id: 2, title: "Step 2: Enter Folder", desc: "Navigate into your new project folder.", hint: "cd myapp" },
                { id: 3, title: "Step 3: Create Files", desc: "Create index.js and Dockerfile (Simulated).", hint: "touch index.js Dockerfile" },
                { id: 4, title: "Step 4: Build Image", desc: "Build your Node.js image.", hint: "docker build -t myapp:latest ." },
                { id: 5, title: "Step 5: Run Local", desc: "Run the container to test it.", hint: "docker run --name myapp_container myapp:latest" },
                { id: 6, title: "Step 6: Create Web Repo", desc: "Create an ECR repository for the website.", hint: "aws ecr create-repository --repository-name mywebsite --region us-east-1" },
                { id: 7, title: "Step 7: Login to ECR", desc: "Authenticate Docker with AWS.", hint: "aws ecr get-login-password | docker login" },
                { id: 8, title: "Step 8: Tag Image", desc: "Tag the website image for ECR.", hint: "docker tag mywebsite:latest 196047493457.dkr.ecr.us-east-1.amazonaws.com/mywebsite:latest" },
                { id: 9, title: "Step 9: Push to Cloud", desc: "Upload image to AWS ECR.", hint: "docker push 196047493457.dkr.ecr.us-east-1.amazonaws.com/mywebsite:latest" },
                { id: 10, title: "Step 10: Switch Device", desc: "Switch to Student Laptop.", hint: "switch-device student-laptop" },
                { id: 11, title: "Step 11: Pull from Cloud", desc: "Download the image on the student laptop.", hint: "docker pull 196047493457.dkr.ecr.us-east-1.amazonaws.com/mywebsite:latest" },
                { id: 12, title: "Step 12: Run on Student Laptop", desc: "Run the website from the cloud image.", hint: "docker run -d -p 8082:80 196047493457.dkr.ecr.us-east-1.amazonaws.com/mywebsite:latest" }
            ]
        }
    };

    // expose globals for the buttons
    window.startAwsMission = () => { navigateTo('simulator'); loadScenario('aws'); };
    window.startEcrMission = () => { navigateTo('simulator'); loadScenario('ecr'); };
    window.startMasterclassMission = () => { navigateTo('simulator'); loadScenario('masterclass'); };
    ...
    function loadScenario(scenarioName) {
        ...
        if (scenarioName === 'masterclass') introText = "Mission: Complete the Full ECR Masterclass Workflow.";
    ...
    function updatePrompt() {
        ...
        if (isConnectedToEc2) {
            if (currentScenario === 'aws') { p = "[ec2-user@ip-172-31-0-1 ~]$"; t = "ec2-user@ip-172-31-0-1: ~"; }
            else if (currentScenario === 'ecr') { p = "root@prod-server:~#"; t = "root@prod-server: ~"; }
            else if (currentScenario === 'masterclass') { p = "student@laptop-b:~$"; t = "student@laptop-b: ~"; }
        }
    ...
    function processCommand(cmd) {
        ...
        if (base === 'help') { writeLine('Available commands: docker, ls, cat, clear, help, ssh, sudo, aws'); return; }
        ...
        if (currentScenario === 'ecr') {
            ...
            if (currentStep === 2 && cmd.includes('aws ecr get-login-password')) {
                writeLine('Login Succeeded'); checkStep(2); return;
            }
            if (currentStep === 3 && base === 'docker' && parts[1] === 'tag') { checkStep(3); return; }
            ...
            if (currentStep === 5 && base === 'ssh' && parts[1] === 'production-server') {
                ...
                isConnectedToEc2 = true; updatePrompt(); checkStep(5); return;
            }
        ...
        if (currentScenario === 'aws') {
            if (!isConnectedToEc2 && currentStep === 2 && base === 'aws' && parts[1] === 'ec2' && parts[2] === 'run-instances') {
                writeLine('Instance launched successfully. Public IP assigned: 54.123.45.67');
                checkStep(2); return;
            }
            if (!isConnectedToEc2 && currentStep === 1 && base === 'aws' && parts[1] === 'configure') {
            ...
            if (!isConnectedToEc2 && base === 'ssh') {
                if (cmd.includes('key.pem') && cmd.includes('@')) {
                    ...
                    isConnectedToEc2 = true; updatePrompt(); checkStep(3); return;
                } else {
                    writeLine('ssh: Could not resolve hostname: Name or service not known'); return;
                }
            }
            if (isConnectedToEc2) {
                if (base === 'ls') { writeLine('docker-compose.yaml  frontend/  backend/  README.md'); if (currentStep === 4) checkStep(4); return; }
                if (base === 'sudo' && parts[1] === 'docker-compose' && parts[2] === 'up') {
                    writeLine('Creating network "my-fullstack-app_default" with the default driver');
                    writeLine('Creating db ... done');
                    state.containers = [
                        { id: 'a1b2c3d4e5', image: 'my-app-frontend', command: 'nginx -g', created: 'Just now', status: 'Up 2s', ports: '0.0.0.0:80->80/tcp' },
                        { id: 'f6e5d4c3b2', image: 'my-app-backend', command: 'node server.js', created: 'Just now', status: 'Up 2s', ports: '3000/tcp' },
                        { id: '1a2b3c4d5e', image: 'mongo:latest', command: 'docker-entrypoint...', created: 'Just now', status: 'Up 2s', ports: '27017/tcp' }
                    ];
                    checkStep(5); return;
                }
                if (base === 'exit') {
                    isConnectedToEc2 = false; writeLine('logout'); updatePrompt(); return;
                }
            }
        }
        ...
        if (currentScenario === 'masterclass') {
            if (currentStep === 1 && base === 'mkdir' && parts[1] === 'myapp') { checkStep(1); return; }
            if (currentStep === 2 && base === 'cd' && parts[1] === 'myapp') { checkStep(2); return; }
            if (currentStep === 3 && base === 'touch') { writeLine('Created index.js'); writeLine('Created Dockerfile'); checkStep(3); return; }
            if (currentStep === 4 && base === 'docker' && parts[1] === 'build') {
                writeLine('Building myapp:latest...'); writeLine('Successfully tagged myapp:latest'); checkStep(4); return;
            }
            if (currentStep === 5 && base === 'docker' && parts[1] === 'run') { writeLine('Hello from Docker + AWS ECR!'); checkStep(5); return; }
            if (currentStep === 6 && cmd.includes('aws ecr create-repository')) {
                writeLine('{ "repository": { "repositoryUri": "196047493457.dkr.ecr.us-east-1.amazonaws.com/mywebsite" } }');
                checkStep(6); return;
            }
            if (currentStep === 7 && cmd.includes('aws ecr get-login-password')) { writeLine('Login Succeeded'); checkStep(7); return; }
            if (currentStep === 8 && base === 'docker' && parts[1] === 'tag') { checkStep(8); return; }
            if (currentStep === 9 && base === 'docker' && parts[1] === 'push') {
                writeLine('The push refers to repository [196047493457.dkr.ecr.us-east-1.amazonaws.com/mywebsite]');
                writeLine('Layer A: Pushed');
                
                const packet = document.getElementById('demo-packet');
                if (packet) { packet.classList.remove('animate-pull'); packet.classList.add('animate-push'); }
                
                checkStep(9); return;
            }
            if (currentStep === 10 && base === 'switch-device') {
                writeLine('Switching to Student Laptop (Laptop B)...');
                isConnectedToEc2 = true; updatePrompt(); checkStep(10); return;
            }
            if (currentStep === 11 && base === 'docker' && parts[1] === 'pull') {
                writeLine('Pulling from 196047493457.dkr.ecr.us-east-1.amazonaws.com/mywebsite');
                writeLine('Status: Downloaded newer image');
                
                const packet = document.getElementById('demo-packet');
                if (packet) { packet.classList.remove('animate-push'); packet.classList.add('animate-pull'); }

                checkStep(11); return;
            }
            if (currentStep === 12 && base === 'docker' && parts[1] === 'run') {
                writeLine('Starting container mywebsite_from_ecr...'); writeLine('Container ID: abc123456789'); checkStep(12); return;
            }
        }
        ...
        if (base === 'docker') {
            if (currentScenario === 'aws' && isConnectedToEc2) {
                 writeLine('Try "sudo docker" instead.'); return;
            }
            handleDockerCommand(parts.slice(1)); return;
        }

        if (base === 'ls') { writeLine('Dockerfile  package.json  src/  README.md'); return; }
        if (base === 'mkdir') { parts[1] ? writeLine('') : writeLine('mkdir: missing operand'); return; }
        if (base === 'cd') { parts[1] ? writeLine('') : null; return; }
        if (base === 'touch') { parts[1] ? writeLine('') : writeLine('touch: missing file operand'); return; }
        if (base === 'pwd') { writeLine('/home/user'); return; }
        if (base === 'sudo' && parts[1] === 'docker') { handleDockerCommand(parts.slice(2)); return; }

        writeLine(`bash: ${base}: command not found`);
    }

    function handleDockerCommand(args) {
        if (args.length === 0) { writeLine('Usage: docker [OPTIONS] COMMAND'); return; }
        const subCmd = args[0];
        ...
        if (subCmd === 'run') {
            ...
            if (image.includes('hello-world')) {
                ...
                if (args.includes('-d')) {
                    ...
                    writeLine(id);
                    state.containers.push({ id: id.substring(0, 12), image: 'nginx', command: '/docker-entry...', created: 'Just now', status: 'Up 1s', ports: '0.0.0.0:8080->80/tcp' });
            ...
            writeLine('CONTAINER ID   IMAGE         COMMAND                  CREATED          STATUS                      PORTS');
            const list = showAll ? state.containers : state.containers.filter(c => c.status.startsWith('Up'));
            list.forEach(c => {
                writeLine(`${c.id}   ${c.image.padEnd(13)} ${c.command.padEnd(24)} ${c.created.padEnd(16)} ${c.status.padEnd(27)} ${c.ports}`);
            });
            if (currentScenario === 'local') {
                if (showAll && currentStep === 2) checkStep(2);
                if (!showAll && currentStep === 4) checkStep(4);
            }
            if (currentScenario === 'aws' && currentStep === 6 && state.containers.length >= 3) checkStep(6);
        }
...
function setupDockerfileInteractions() {
    window.highlightLine = function(keyword) {
        const explanations = {
            ...
            'EXPOSE': 'EXPOSE: Informs Docker that the container listens on the specified network ports.',
